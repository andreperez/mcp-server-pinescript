...
export async function quickValidateLineContinuation(source) {
	...
	// Track parenthesis depth across lines.
	// Dec 2025 Pine Script v6: any indentation is allowed for wrapped lines inside parentheses,
	// so we must not flag operator-terminated lines that are already inside open parens.
	let parenDepth = 0;
	...
	for (let i = 0; i < lines.length; i++) {
		...
		// Record paren depth BEFORE processing this line, then update for subsequent lines.
		const entryParenDepth = parenDepth;
		parenDepth += (line.match(/\(/g) || []).length;
		parenDepth -= (line.match(/\)/g) || []).length;
		if (parenDepth < 0) parenDepth = 0;

		// Skip operator-continuation checks when:
		//   1. The line contains a switch arm `=>` — the `>` character incorrectly matches
		//      the comparison operator pattern, producing false positives.
		//   2. The line is entered while inside open parentheses (Dec 2025 update).
		const isSwitchArmLine = /=>/.test(trimmedLine);
		if (!isSwitchArmLine && entryParenDepth === 0) {
		...
		} // end operator-continuation check guard (!isSwitchArmLine && entryParenDepth === 0)
